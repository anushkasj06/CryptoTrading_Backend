...
//    }
    ...
    // WalletController.java (Corrected walletToWalletTransfer method)
    ...
    public ResponseEntity<Wallet> walletToWalletTransfer(@RequestHeader("Authorization")String jwt,
        ...
        // 1. Perform balance transfer in the service (updates balances for both)
        Wallet senderWallet = walleteService.walletToWalletTransfer(senderUser,reciverWallet, req.getAmount());

        // 2. RECORD TRANSACTION FOR SENDER (Debit)
        walletTransactionService.createTransaction(
                senderWallet,
                WalletTransactionType.WALLET_TRANSFER,
                reciverWallet.getId().toString(), // Transfer ID points to the recipient
                req.getPurpose(),
                -req.getAmount() // Sender DEBIT
        );

        // 3. ✅ ADDED: RECORD TRANSACTION FOR RECEIVER (Credit)
        walletTransactionService.createTransaction(
                reciverWallet,
                WalletTransactionType.WALLET_TRANSFER,
                senderWallet.getId().toString(), // Transfer ID points back to the sender
                "Transfer received: " + req.getPurpose(), // Updated purpose for clarity
                req.getAmount() // Receiver CREDIT
        );
